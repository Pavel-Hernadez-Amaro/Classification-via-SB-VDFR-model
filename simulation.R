...
mean2  <- rev(mean1)
coef   <- rep(0.3,nbasis)
# coef <- .2*c( sort(seq(1,30, length.out = nknots/3), decreasing = TRUE) ,rep(5,(nknots/3)+2), seq(1,30, length.out = nknots/3))
...
probs_sample=array(dim=c(n,n-1,maxl+1,S)) # probabilities of the logistic model

probs_pred=array(dim=c(n,maxl+1,S))
...
clas_pred=optimal_cut_SB_VDFR=AUC_SB_VDFR = array(dim=c(n,maxl+1,S))

min_track=array(dim=c(S,maxl+1))

optimal_basis = array(dim = c(maxl+1,2,S))

c1=80 # number of basis for the SB_VDFR model
c2=80
sub=500
...
for (s in 1:S) {
  ...
  ao=all.together.splines.last(knots, mean1, err1, mean2, err2, n1, n2, p, grid, maxi, maxl, c1, c2, sub, k, lambda.sm, lambda.buja, coef, unif.miss=unif.miss, rate=NA)
  ...
  probs_pred[,,s]=ao$prob_SB_VDFR_pred
  min_track[s,]=ao$min_track
  clas_pred[,,s]=ao$clas_pred
  probs_sample[,,,s]=ao$prob_SB_VDFR_sample
  optimal_cut_SB_VDFR[,,s] = ao$optimal_cut
  AUC_SB_VDFR[,,s] = ao$AUC
...
aux=as.numeric(probs_pred[,cut_case,s]>=optimal_cut_SB_VDFR[,cut_case,s])

confusionMatrix(as.factor(aux),as.factor(y))
...
all.equal(length(which(!is.na(x[indx_aux,]))),length(SB_VDFR_Class$x_h[[indx_aux]]))
